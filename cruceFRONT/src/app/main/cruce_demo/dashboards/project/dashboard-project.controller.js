(function ()
{
    ...
    angular
        ...
        .controller('DashboardProjectController2', DashboardProjectController2);

    /** @ngInject */
    function DashboardProjectController2( URLService, $state, $http, $timeout, localStorageService, $mdDialog, $scope, $document, $interval, $mdSidenav, DashboardData)
    {
        var vm = this;
        ...
        function inicio () {
            var peticion={
            method: 'GET',
            url: vm.url+'/cruceAPI/public/dashboard'
            }
            $timeout(function() {
                $http(peticion).then(function(response) {
                    console.log(response);
                    vm.inspectores=response.data.clientes;
                    vm.part1=response.data.participaciones1;
                    vm.part2=response.data.participaciones2;
                    vm.part3=response.data.participaciones3;
                    vm.part4=response.data.participaciones4;
                    vm.est_SorteoWeb_x=response.data.Estadisticas_SorteoWeb_x;
                    vm.est_SorteoWeb_y=response.data.Estadisticas_SorteoWeb_y;
                    vm.est_ParseEmails_x=response.data.Estadisticas_ParseEmails_x;
                    vm.est_ParseEmails_y=response.data.Estadisticas_ParseEmails_y;
                    vm.est_Cleaned_members_x=response.data.Estadisticas_Cleaned_members_x;
                    vm.est_Cleaned_members_y=response.data.Estadisticas_Cleaned_members_y;
                    vm.est_Subscribed_members_x=response.data.Estadisticas_Subscribed_members_x;
                    vm.est_Subscribed_members_y=response.data.Estadisticas_Subscribed_members_y;
                    vm.N_SorteoWeb=response.data.N_SorteoWeb;
                    vm.N_Parse=response.data.N_Parse;
                    vm.N_Cleaned_members=response.data.N_Cleaned_members;
                    vm.N_Subscribed_members=response.data.N_Subscribed_members;

                    vm.polarChart = {
                        labels: ['Mongo mensajes', 'Sorteo web', 'Cleaned members', 'Subscribed members'],
                        data  : [vm.N_Parse, vm.N_SorteoWeb, vm.N_Cleaned_members, vm.N_Subscribed_members]
                    };

                    vm.lineChart = {
                        labels: vm.est_SorteoWeb_x,
                        series: ['Sorteo web'],
                        colours: ["#808080"],
                        data  : [
                            vm.est_SorteoWeb_y
                    ...
                    vm.lineChart2 = {
                        labels: vm.est_ParseEmails_x,
                        series: ['Mongo mensajes'],
                        colours: ["#FF0000"],
                        data  : [
                            vm.est_ParseEmails_y
                        ]
                    };

                    vm.lineChart3 = {
                        labels: vm.est_Cleaned_members_x,
                        series: ['Cleaned members'],
                        colours: ["#32CD32"],
                        data  : [
                            vm.est_Cleaned_members_y
                        ]
                    };

                    vm.lineChart4 = {
                        labels: vm.est_Subscribed_members_x,
                        series: ['Subscribed members'],
                        colours: ["#FFFF00"],
                        data  : [
                            vm.est_Subscribed_members_y
                        ]
                    };
